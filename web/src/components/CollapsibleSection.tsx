/**
 * Collapsible left-pane section + the shared open/closed store behind it.
 *
 * Open-state overrides persist to localStorage (SECTIONS_STORAGE_KEY); a
 * section with no stored override falls back to its `defaultOpen`. Content
 * stays MOUNTED while collapsed (hidden via the `hidden` attribute) so
 * react-query polling and live count badges keep working off-screen.
 */
import { createContext, useContext, useEffect, useMemo, useState, type ReactNode } from 'react';
import { readJson, writeJson } from '../lib/storage';

export const SECTIONS_STORAGE_KEY = 'crossex:sections:v1';
...
interface SectionProps {
  id: string;
  title: string;
  /** Live count chip etc. — rendered inside the header button. */
  badge?: ReactNode;
  defaultOpen: boolean;
  children: ReactNode;
}

export function CollapsibleSection({ id, title, badge, defaultOpen, children }: SectionProps) {
